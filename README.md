...
这个插件用于将蓝光原盘重封装为 MKV，支持两种模式：

- 整理历史重封装：读取 MoviePilot 最近整理历史，在媒体库已入库的原盘目录中重封装 MKV。
- 监听下载器重封装：在 MoviePilot 整理下载器原盘任务前拦截，先在下载目录重封装 MKV，再用原下载历史中的媒体 ID 发起 MKV 整理。

大致流程：

1. 查找需要处理的蓝光原盘目录。
2. 使用 MakeMKV 提取最长正片并生成 MKV。
